...
import python.ivector as iv
import python.gmm as gmm
import python.energy_vad as evad
import python.ivector_io as ivio
...
fbank_mx      = python.features.mel_fbank_mx(winlen_nfft = WINDOWSIZE/SOURCERATE,
                                      fs          = fs,
                                      NUMCHANS    = NUMCHANS,
                                      LOFREQ      = LOFREQ,
                                      HIFREQ      = HIFREQ)
...
class IVector():
    ...
    def process_wav(self, wav_file, mode="ivector", vad_dir="auto"):
        if mode not in ["ivector", "statistics", "mfcc"]:
            ...
            if ADDDITHER > 0.0:
                #print('  Adding dither')
                sig = python.features.add_dither(sig, ADDDITHER)

           
            fea = python.features.mfcc_htk(sig,
            ...
            # [add_deriv] step
            fea = python.features.add_deriv(fea, (deltawindow, accwindow))
            ...
            if vad_dir == "auto":
                ...
                fea = python.features.cmvn_floating(fea, cmvn_lc, cmvn_rc, unbiased=True)
