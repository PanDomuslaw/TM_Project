...
def compute_vad(s, win_length=160, win_overlap=80, n_realignment=5, threshold=0.3):
    ...
    F = python.features.framing(s, win_length, win_length - win_overlap) 
    ...
    GMM = python.gmm.gmm_eval_prep(ww, mm, ee)

    E = E[:,np.newaxis]

    for i in range(n_realignment):
        # collect GMM statistics
        llh, N, F, S = python.gmm.gmm_eval(E, GMM, return_accums=2)

        # update model
        ww, mm, ee   = python.gmm.gmm_update(N, F, S)

        # wrap model
        GMM = python.gmm.gmm_eval_prep(ww, mm, ee)

    # evaluate the gmm llhs
    llhs = python.gmm.gmm_llhs(E, GMM)

    llh  = python.gmm.logsumexp(llhs, axis=1)[:,np.newaxis]
